...
class RGBDepthSubscriber:
    def __init__(self):
        rospy.init_node('face_weapon_processor') #, anonymous=True)
        ...
        self.rgb_sub = rospy.Subscriber("/depth/RGB_image", Image, self.rgb_callback, queue_size = 1)
        time.sleep(1)
        self.face_bbox = rospy.Subscriber("/YOLO/face_bbox", Float32MultiArray, self.process_face, queue_size = 1)
        self.person_bbox_with_id = rospy.Subscriber("/SORT/person_bbox_with_id", Float32MultiArray, self.face_in_personbox, queue_size = 1)
        self.weapon = rospy.Subscriber("/YOLO/weapon_bbox", Float32MultiArray, self.weapon_in_personbox, queue_size = 1)
    ...
    def process_face(self, msg): 
        ...
        for i in range(len(msg.data)//6):
            ...
            shape = sp(image, dlib.rectangle(x1+x_offset, y1 + y_offset, x2-x_offset, y2))
            ...
            #if dist < 0.396 and not self.face_dist: # me 
                #print("my face detected")
                self.face_dist.append([x1, y1, x2, y2])

            time.sleep(max(0.0, 0.022 - time.time() + a))
    ...
    def weapon_in_personbox(self, msg):   # with high confidennce threshold
        for i in range(len(msg.data)//6):
            #self.win = self.rgb_image
            det = msg.data[6*i : 6*(i+1)-2]
            x1, y1, x2, y2 = map(int, det)
            self.weaponbox.append([(x1+x2)//2, (y1+y2)//2])
            
    def face_in_personbox(self, msg):
        x1, y1, x2, y2, id = map(int, msg.data[:])
        if self.face_dist:
            for [a, b, c, d] in self.face_dist:
                #a, b, c, d = map(int, self.face_dist[:])
                if x1 <= a and y1 <= b and x2 >= c and y2 >= d:
                    #print("my face detected")
                    self.me_list.append(id)
        ...
        if self.weaponbox:
            ...
            for [cx, cy] in self.weaponbox:
                if cx <= x2 and cx >= x1 and cy <= y2 and cy >= y1 and id not in self.me_list:
                    #print("dangerous person detected!, id is ", id)
                    self.is_dangerous_person.publish(id)
                    # self.target_bbox_msg.data = (x1, y1, x2, y2)
                    # self.target_pub.publish(self.target_bbox_msg)
                    self.weaponbox.clear()
                    break
            if len(self.weaponbox) > 2:
                self.weaponbox.clear()
