...
# make realsense publisher

rgb_queue = queue.Queue(maxsize=10000000)
depth_queue = queue.Queue(maxsize=10000000)
# Initialize the CvBridge to convert images
bridge = CvBridge()
    ...
    try:
        while not rospy.is_shutdown():
            ...
            if color_frame and depth_frame:
                ...
                #depth_image = bridge.imgmsg_to_cv2(depth_frame, "16UC1")

                # Put frames into the queue
                # if not depth_queue.full():
                #     depth_queue.put(depth_image) # publlish only depth ??????????????????????
                if not rgb_queue.full():
                    rgb_queue.put(color_image) # publlish only depth ??????????????????????
...
def publish_frames():
    ...
    #rospy.init_node('custom_realsense_publisher', anonymous=True)
...
class LoadStreams:
    ...
    def __init__(self, sources='file.streams', imgsz=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            self.imgs[i] = rgb_queue.get() #cap.read()  # guarantee first frame
